 ...
 * 按内容特征分类记忆类型。
 */
function classifyMessages(messages: any[]): Array<{ type: MemoryType; content: string }> {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (/决定|选择|采用|改用|方案|决策/i.test(content) && content.length < 300) {
      results.push({ type: "decision", content: content.slice(0, 200) })
      continue
    }

    // 识别工作模式/经验（包含"先.*再"、"需要"、"建议"、"注意"等）
    if (/(先.*再|需要|建议|注意|必须|不要|避免)/i.test(content) && content.length < 200) {
      results.push({ type: "pattern", content: content.slice(0, 200) })
      continue
    }

    // 识别经验教训（包含"报错"、"失败"、"问题"、"修复"等）
    if (/(报错|失败|问题.*解决|修复|错误|bug|教训)/i.test(content) && content.length < 200) {
